...
namespace FinalYearProject.Areas.Staff.Controllers
{
    ...
    public class PayrollController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            return View(await _db.Payroll.Include(p=>p.PayBackDetails).Include(p=>p.SalaryAdvanceDetails).ToListAsync());
        }
        ...
        public async Task<IActionResult> CreatePayroll(Models.Payroll payroll, DateTime start_date, DateTime end_date)
        {
            ...
            foreach (var sa in salaryAdvance)
            {
                if (sa != null)
                {
                    ...
                    for (int i = 0; i < sa.time_to_payback; i++) {

                        PayBack pb = new PayBack()
                        {
                            payback_id = GeneratePayBackID().Result,
                            advance_id = sa.advance_id,
                            payback_amount = sa.amount / sa.time_to_payback,
                            status = "Not Paid"
                        };
        ...
        public async Task<string> GeneratePayBackID()
        {
            ...
            var payback = await _db.PayBack
             .OrderByDescending(s => s.payback_id)
             .FirstOrDefaultAsync();

            if (payback != null)
            {               
                int lastIdNumericPart = int.Parse(payback.payback_id.Substring(2));
        ...
        public async Task<IActionResult> DeleteConfirmed(string id)
        {
            ...
            var payroll =  _db.Payroll.Include(p => p.EmployeeDetails).Where(p => p.payroll_id == id).FirstOrDefault();
            ...
            if (payroll != null)
            {
                ...
                if (salaryAdvance!= null && payBack !=null)
                {
                    ...
                    if (findPayBack.Count() == payBack.Count() && salaryAdvancePaid != null) {
                        ...
                        var findPayback = _db.PayBack.Where(pb => pb.advance_id == salaryAdvance.advance_id);

                        _db.RemoveRange(findPayback);
                        await _db.SaveChangesAsync();
                    }

                    var salaryAdvanceTimePayBack1 = await _db.SalaryAdvance.Where(sa => sa.advance_id == payroll.advance_id
                         && sa.status == "paying back" && sa.time_to_payback==1).FirstOrDefaultAsync();
                    if (salaryAdvanceTimePayBack1 != null)
                    {
                        salaryAdvanceTimePayBack1.status = "approved";
                        _db.SalaryAdvance.Update(salaryAdvanceTimePayBack1);

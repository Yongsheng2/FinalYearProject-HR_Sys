    ...
    public class ScheduleJob : CronJobService
    {
        ...
        public async void initializeRole()
        {
            using (var scope = _sFactory.CreateScope())
            {
                ...
                var surveyMange = await _roleManager.FindByNameAsync(SD.SurveyManage);

                if (surveyMange == null)
                {
                    await _roleManager.CreateAsync(new IdentityRole(SD.SurveyManage));
                }

                var surveyView = await _roleManager.FindByNameAsync(SD.SurveyView);

                if (surveyView == null)
                {
                    await _roleManager.CreateAsync(new IdentityRole(SD.SurveyView));
                }

                var companyKPIs = await _roleManager.FindByNameAsync(SD.CompanyKPIs);

                if (companyKPIs == null)
                {
                    await _roleManager.CreateAsync(new IdentityRole(SD.CompanyKPIs));
                }

                var incentives = await _roleManager.FindByNameAsync(SD.Incentives);

                if (incentives == null)
                {
                    await _roleManager.CreateAsync(new IdentityRole(SD.Incentives));
                }

                var salaryAdvanceManage = await _roleManager.FindByNameAsync(SD.SalaryAdvanceManage);

                if (salaryAdvanceManage == null)
                {
                    await _roleManager.CreateAsync(new IdentityRole(SD.SalaryAdvanceManage));
                }

                var salaryAdvance = await _roleManager.FindByNameAsync(SD.SalaryAdvance);

                if (salaryAdvance == null)
                {
                    await _roleManager.CreateAsync(new IdentityRole(SD.SalaryAdvance));
                }

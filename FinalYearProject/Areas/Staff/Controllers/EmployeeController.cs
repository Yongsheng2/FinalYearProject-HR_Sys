...
namespace FinalYearProject.Areas.Staff.Controllers
{
    ...
    public class EmployeeController : Controller
    {
        ...
        [ActionName("GetEmployer")]
        public async Task<IActionResult> GetEmployer(string id)
        {
            var employers = await (from employee in _db.EmployeeDetails where employee.parent_company == id select employee).ToListAsync();

            return Json(new SelectList(employers.AsEnumerable(), "employee_id", "employee_name"));
        }
